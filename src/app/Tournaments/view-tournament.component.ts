...
export class ViewTournamentComponent implements OnInit {
    ...
    findWins(id): number {
        const record = this.records.find((eachRecord) => JSON.stringify(eachRecord.team) === JSON.stringify(id));
        if (record !== undefined) {
            return record.wins;
        }
        return 0;
    ...
    calculateRecords() {
        for (const game of this.playedGames) {
            if (game.winner) {
                const winnerIndex = this.records.indexOf(this.records.find((record) => JSON.stringify(record.team) === JSON.stringify(game.winner)));
                ...
                const loserIndex = this.records.indexOf(this.records.find((record) => JSON.stringify(record.team) === JSON.stringify(loser)));
