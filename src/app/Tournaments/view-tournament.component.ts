...
export class ViewTournamentComponent implements OnInit {
    ...
    filterGames(input, list) {
        const matchingPlayers = this.players.filter((value) => {
        ...
        if (list === 'unplayed') {
            this.unplayedGames = this.games.filter((game) => {
                    ...
                    if (this.tournyType === 'doubles') {
                        return (matchingIds.includes(game.team1[0]) || matchingIds.includes(game.team1[1])
                          || matchingIds.includes(game.team2[0]) || matchingIds.includes(game.team2[1])) && !game.winner;
                    }
    ...
    findWins(id): number {
        const record = this.records.find((record) => JSON.stringify(record.team) === JSON.stringify(id));
        if (record !== undefined) {
            return record.wins;
        }
        return 0;
    ...
    calculateRecords() {
        for (const game of this.playedGames) {
            if (game.winner) {
                const winnerIndex = this.records.indexOf(this.records.find((record) =>
                  JSON.stringify(record.team) === JSON.stringify(game.winner)));
                ...
                const loserIndex = this.records.indexOf(this.records.find((record) =>
                  JSON.stringify(record.team) === JSON.stringify(loser)));
    ...
    sortPools() {
        for (const pool of this.idPools) {
            pool.sort((a, b) => {
                    const aWins = this.findWins(a);
                    const bWins = this.findWins(b);
                    const aDiff = this.findDifferential(a);
                    const bDiff = this.findDifferential(b);
                    if (aWins > bWins) {
                        return -1;
                    } else if (aWins === bWins) {
                        if (aDiff >= bDiff) {
    ...
    generateStandings() {
        for (const pool of this.idPools) {
            ...
            for (const team of pool) {
                this.records.push({'team': team, 'wins': 0, 'losses': 0, 'totalDifferential': 0,
                  'pool': this.idPools.indexOf(pool), 'parent': 0, 'playoffSeed': 0});
            }
        }
    ...
    addToPlayoffs(team) {
        if (this.playoffPool.find((record) => JSON.stringify(record.team) === JSON.stringify(team)) === undefined) {
            const record = this.records.find((record) => JSON.stringify(record.team) === JSON.stringify(team));
            this.playoffPool.push(record);
        } else {
            const index = this.playoffPool.findIndex((record) => JSON.stringify(record.team) === JSON.stringify(team));
    ...
    constructBracket() {
        const bracket = this._bracketService.generateBracket(this.playoffPool);
        ...
        const playInSpots = [];
        for (const spot of this.treeLevels[1]) {
    ...
    startPlayoffs() {
        ...
        let seed = 1;
        for (const team of this.playoffPool) {
